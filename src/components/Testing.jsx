...
export default function Testing({ isVisible, showModal }) {
  ...
  const [desToken, setDesToken] = useState(null);
  ...
  const switchNetworkToOpBNB = async (prov) => {
    ...
    // console.log(prov.isMetaMask);
    // console.log(prov.isSafePal);
    try {
      await prov.request({
        method: "wallet_switchEthereumChain",
    ...
    if (isConnected && toToken) {
      ...
      if (lastEdited == "from") {
        ...
        if (parseFloat(fromAmount) > 0) {
          ...
          setToAmount(parseFloat(toValue).toFixed(4));
        }
        // console.log(result);
  ...
  const handleSwapButtonClick = async () => {
    ...
    try {
      ...
      // here to write the logic of the swap from bnb to the angh netwrok and visa versa
      // await switchNetworkToOpBNB(provider2 || window.ethereum);
      // console.log(zeroAddress);
      // console.log({ tokenA, tokenB, ZeroAddress });
      // const hash =
      //   "0x588efb0e7d6ac8e303c302a46c60bf75b122c93b75ea99421388c1e377a778ec";

      // // Remove "0x" and convert to bytes
      // const bytes = Uint8Array.from(Buffer.from(hash.slice(2), "hex"));

      // // Encode to base32
      // const base32Hash = base32.encode(bytes);
      ...
      if (tokenA.name == "USDT-OpBNB") {
        ...
        await switchNetworkToAngh(provider2 || window.ethereum);
        //now from here the api will start working
      } else {
        await switchNetworkToAngh(provider2 || window.ethereum);
  ...
  async function hitAfterDelay() {
    ...
    const res = await axios.post("https://anghswap.info:3535/verify-and-sign", {
      srcChain: sourceChain,
      txHash: transactionHash,
      destChain: destinationChain,
      desToken,
  ...
  async function claimTokens() {
    const tokenB = getTokenBySymbol(toToken);
    console.log({ datas, tokenB, toAmount });
    if (destinationChain === "opbnb") {
      await switchNetworkToOpBNB(provider2 || window.ethereum);
  ...
  return (
    <>
      <div
        ...
        } mt-8 lg:mt-0 flex flex-col gap-10 justify-center lg:justify-end`}
      >
    ...
    ${
      ...
      !fetchingPrice
        ...
        {transactionHash && (
          <div>
            {transactionHash}, {sourceChain}, {destinationChain}, {signature}
          </div>
        )}
        ...
        <div>
          <button onClick={claimTokens}>hit Request</button>
        </div>
      </div>
